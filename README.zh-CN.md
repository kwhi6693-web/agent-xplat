...
当前验证路径会在真实的 GitHub-hosted Windows、macOS 和 Linux runner 上运行。静态分析覆盖完整的 8 个目标矩阵；静态发现使用 `STATIC = INFERRED`，只有真实 runner 或受控本地运行时检查才会产生 `RUNTIME = VERIFIED` 证据。
...
要求 Python 3.10 或更高版本。package 已以 wheel 和 source distribution 形式发布到 PyPI。在 active virtual environment 中使用 `python -m pip install agent-xplat`，或使用 `pipx install agent-xplat` 进行隔离的 CLI 安装。运行时 package 包含 JavaScript/JSX/TypeScript/TSX AST 分析所需的轻量 Tree-sitter parser bindings；`pytest` 仅属于 development extra。从 source checkout 工作时，`pipx install .` 仍是方便的隔离 CLI 安装方式。

从 source checkout 安装：
...
也始终可以使用 module invocation 作为 fallback：
...
默认的受限发现流程包括 `SKILL.md`、`AGENTS.md`、`CLAUDE.md`、`GEMINI.md`、`README.md`、`.github/**`、`.cursor/**`、`.claude/**`、`.codex/**`、`scripts/**`、package manifest/lockfile、Python metadata、Docker/Make 文件，以及常见的 Shell、Python、JavaScript、JSX、TypeScript、TSX 和 batch 扩展名（`.js`、`.mjs`、`.cjs`、`.jsx`、`.ts`、`.mts`、`.cts`、`.tsx`）。`.git`、`node_modules`、`vendor`、`dist`、`build`、缓存、二进制文件和超大文件会被排除。

## ✨ 示例

内置 fixture 覆盖 portable、OS-specific、shell-specific、Python、Node、mixed 和 agent-instruction 工作流：
...
`tests/fixtures/*/expected.json` 中的 fixture metadata 记录预期规则、受影响 target、severity 和 confidence。这些是测试数据，不是对第三方工具标准的声明。

## 🧩 规则、严重级别与置信度

当前 registry 包含 49 条模块化规则，并使用稳定的 `AX-*` identifier。规则覆盖路径、Shell 命令与环境变量语法、quoting、Python、Node package script 和 Node/JS/TS AST 事实、文件系统、包管理器、外部工具、运行时假设及 Agent 配置。JavaScript 系列 source 会使用 Tree-sitter 做结构化解析；动态字符串和行为仍然属于静态推断。详见 [docs/RULES.md](docs/RULES.md)。

Severity 为 `BLOCKER`、`ERROR`、`WARNING` 或 `INFO` 之一。Confidence 为 `HIGH`、`MEDIUM` 或 `LOW` 之一。低置信度假设会明确标记为低置信度，不会仅仅因为不方便就升级为 blocker。
...
Ignored findings 仍会以 `ignored: true` 保留在 machine-readable output 中，summary 也会报告其数量。未知 key、target、severity、rule ID 和无效值都会以 exit code 2 失败。未使用的行级 suppression marker 会作为 suppression diagnostic 报告，而不会被静默接受。

## 🤖 Agent-native 使用与 exit code

```bash
agent-xplat scan . --format json
```

Agent 应消费 `summary`、每个 target 的 `scores`、`findings` 和 `contract.violations`，不要解析 terminal decoration。CLI 命令包括 `scan`、`test`、`fix`、`report`、`explain`、`doctor`、`baseline`、`init`、`init-ci` 和 `badge`；root command 还支持 `--version`。
...
静态分析无法证明每一种 Shell 版本、已安装工具、文件系统策略、原生 binary、动态命令字符串或运行时行为。JavaScript 系列 source 已具备支持扩展名的结构化 AST 分析，但动态求值、生成代码、不支持的语法恢复和实际 subprocess 行为仍属于运行时问题。Release workflow 已生成并有文档说明，但 hosted runner 证据必须来自用户自己的 GitHub 仓库。未来可以增加更多 runtime adapter 和经过独立审查的规则，同时保持公开 finding contract 不变。
...
发布路径使用 GitHub Actions Trusted Publishing，即基于 OIDC 的发布流程，不需要长期有效的 PyPI token。[验证记录](docs/audit/verification-run.md) 说明本地检查、托管 Windows/macOS/Linux 证据、package readback，以及 inferred 静态发现和 verified 运行时结果之间的边界。发布变更汇总在 [CHANGELOG.md](CHANGELOG.md)。
