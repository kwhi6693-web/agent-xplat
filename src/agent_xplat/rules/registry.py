...
_RULES: tuple[RuleSpec, ...] = (
    ...
    _spec("AX-NODE-005", "Node child-process shell command", "Detects shell parsing and OS-specific commands embedded in child_process calls.", Severity.ERROR, Confidence.HIGH, "Use an argument-array API, a portable runtime abstraction, or an explicitly selected launcher.", detect_node, "exec(\"rm -rf dist\")", "child_process command string", "The command contract is tied to a shell or operating system.", "The call and literal command are identified from the AST."),
    _spec("AX-NODE-006", "Node environment access without fallback", "Detects process.env reads that rely on an undeclared host environment value.", Severity.WARNING, Confidence.MEDIUM, "Declare the required variable and provide a deterministic default or validation path.", detect_node, "process.env.OUTPUT", "unhandled process.env access", "Host environment state can differ across shells and CI runners.", "The member access is structurally identified; intent still depends on surrounding code."),
    _spec("AX-NODE-007", "Unbalanced Node platform branch", "Detects process.platform usage without a complete alternate branch.", Severity.WARNING, Confidence.MEDIUM, "Provide a fallback branch for every supported target or declare the target restriction.", detect_node, "if (process.platform === \"win32\") runWindows()", "platform branch without fallback", "A one-sided platform branch leaves other operating systems without a proven path.", "The AST exposes both the condition and whether an alternate branch exists."),
    _spec("AX-NODE-008", "Hardcoded Node executable or path", "Detects OS-specific executable names, absolute paths, and separators in child_process calls.", Severity.ERROR, Confidence.HIGH, "Resolve executables through a runtime API and compose paths with path.join/path.resolve.", detect_node, "spawnSync(\"cmd.exe\")", "hardcoded child-process executable", "The executable or separator is unavailable or interpreted differently on another target.", "The literal is directly observed in a bound child_process call."),
